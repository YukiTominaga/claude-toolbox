...
その後、回数 (`max_runs_per_day`) と経過時間 (`max_minutes_per_run`) も同じ理由で撤廃した
(`docs/spec/budget-removal.md` — 対話セッションでは人が見ているため、回数や時間で切っても
暴走は防げず、作業を切られる不便だけが残る)。残る歯止めは内側ループの `max_rounds` /
`max_no_progress`、人が倒す `status: paused`、無人実行のターン数上限
(`loop-run.sh` の定数)の 3 種類で、いずれも課金形態に依存しない。
