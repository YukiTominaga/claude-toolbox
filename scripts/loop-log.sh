...
#   {"event":"start"}   loop-guard.sh が実行開始時に記録する(実行回数の集計対象)
#   {"event":"<結果>"}  このスクリプトが記録する(何がどうなったかの履歴)
# 実行回数を数えるのは start 行だけなので、この呼び出しを忘れても回数は狂わない
# (回数は報告用の実績値であって、ループを止める判定には使わない。
...
if [ "${1:-}" = "--recent" ]; then
  ...
  [ -f "$LEDGER" ] || exit 0
  # **結果行だけ**を新しい順に返す。台帳には集計用の行 (start / cost) も混ざるが、
