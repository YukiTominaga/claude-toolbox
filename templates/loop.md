...
<!-- ここに挙げた操作は、ループが自動で実行してはならない。
     push をゲートに含めるかは環境で判断する。未 push のコミットを差し戻す Stop hook が
     ある場合、push をゲートにしても無人ループでは強制力を持たない。また使い捨ての
     リモート環境では push しないと成果が失われる。不可逆なのは merge であって push
     ではないため、迷うならゲートは merge 側に置く。 -->
