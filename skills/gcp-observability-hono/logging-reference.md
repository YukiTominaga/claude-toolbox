...
|---|---|
| `traceparent` ヘッダーを split して traceId を抽出 | `logger.emit()` が `context.active()` から自動取得 |
...
| Hono `Context` をログ関数に引き回す | 引き回し不要（AsyncLocalStorage 経由） |
...
つまり **`context.with()` の内側にいればトレース相関は自動的に効く**。
...
未設定だと no-op になり**ログが黙って消える**ので、初期化順序（`startTelemetry()` を先に）を守ること。
...
app.post('/something', async (c) => {
  // Hono Context を渡す必要はない。相関はアクティブな span から取られる
...
logger.emit({
  ...
  body: { path: c.req.path, query: c.req.query() },
});
```
