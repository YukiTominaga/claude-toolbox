...
| 記事の要素 | crystal での対応 |
|---|---|
| Automations(定期実行による発見と分類) | **組み込みの `/loop` `/schedule` に委ねる**。本 plugin では実装しない |
...
- **ゴール中は stop-gate も毎ターン走る**: goal-gate が差し戻すと以降の Stop は
  `stop_hook_active: true` になるため、stop-gate をそのまま素通しさせると
  ゴールループ中はテスト・型チェックが一度も走らなくなる。
  そのため stop-gate はゴールが `active` の間だけ素通しを無効にする
- **判定器は会話の出力しか読めない**: コマンドを実行することもファイルを読むこともできない。
  そのため完了条件は `<終了状態> — 検証: <コマンド> が <期待結果>` の形で書き、
  検証コマンドの出力が作業ログに現れていない条件は未達として扱われる。
...
| `/crystal:goal` | 人が 1 回叩く → 以降はターン終了ごとに自動 | 完了条件をすべて満たす / `max_rounds` 超過 | Haiku(goal-gate) | `.claude/goal.md` |
...
| コスト爆発 | judge は Haiku 固定。`--output-format json` の `total_cost_usd` を frontmatter の `cost_usd` に累積し、`/crystal:goal status` で確認できる |

> **`/loop` と goal を同時に使わないこと。** 時間トリガー(loop)とターン終了トリガー(goal-gate)の
> 二重ループになり、loop の 1 周ごとに goal-gate が差し戻しを重ねるため、コスト爆発に直結する。

## 非対話実行

CI やバッチから回す場合も、専用のループスクリプトは要らない:

```bash
claude -p "ゴールを達成するまで作業を続けて"
```

`claude -p` でも Stop hook は発火するため、`.claude/goal.md` が `status: active` なら
goal-gate がそのまま効く。未達なら差し戻され、達成(`status: done`)または
`max_rounds` 超過(`status: stalled`)で終了する。
...
| 完了報告の裏取り | `SubagentStop`(`subagent-gate.sh`) | 「テストが通った」と主張しているのに実行痕跡が無いものだけを止める。安く済む |
...
vitest で `hooks/goal-gate.sh` の振る舞いをテストしている。
