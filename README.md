...
- **免除は機械判定できない。** 既存テストで担保されるリファクタ、仕様が変わらない
  ...
  **1 度だけ差し戻し、自分の差し戻し後の再停止では素通しする**。免除に当たるなら
  その旨を報告に書いてから応答を終えれば通る。「黙って書き忘れる」は止まり、
  「理由を述べて省く」は通る、という線引き
- **`stop_hook_active` だけでは素通ししない。** このフラグは「どれかの Stop hook が
  差し戻した」印であり、自分のものとは限らない。フラグだけで折れると、stop-gate の
  差し戻しがこのゲートの検査を丸ごと免除してしまう。各ゲートは差し戻した事実を
  状態ファイル(`~/.claude/state/crystal/`)に記録し、**自分の記録がある再停止だけ**を
  素通しする。session_id が取れないビルドでは従来どおりフラグで素通しする(fail-open)
- **発火はこのセッションの作業痕跡で絞る。** 作業ツリーはセッション開始前から汚れている
  ことがある(人間の書きかけ、前セッションの残り)。transcript に作業痕跡
  (編集ツール / Bash 書き換え / コードを変更したサブエージェントの記録)が無い
  会話だけのターンでは発火しない。編集ツールの痕跡だけなら、実装の判定は
  そのファイル群に限る(テスト・spec の対はツリー全体から探す。人間が先にテストを
  書いてある TDD を差し戻さないため)。Bash 書き換えやサブエージェントの編集記録が
  あるときは、何を触ったか特定できないのでツリー全体を見る
...
- **`docs/spec/` に仕様が 1 つも無ければ発火しない。** verifier は仕様の受け入れ条件を
  ...
  差し戻しても状況が変わらない = 抜けられないループになる。存在判定は
  `SPEC_RE` と同じくサブディレクトリまで見る(直下しか見ないと、仕様を機能別
  ディレクトリに整理した途端このゲートだけが黙って死ぬ)
...
- **サブエージェントに委譲した実装は取りこぼさない。** メイン transcript には Task の
  起動しか現れないため、そのままでは実装を委譲するだけでこのゲートが沈黙する。
  `record-subagent-edits.sh`(SubagentStop)が「コードを変更したサブエージェントがいた」
  事実を状態ファイルに記録し、記録があるときはサブエージェント起動を変更痕跡として扱う。
  verifier の PASS を受理した時点で記録は消える。どの起動が編集したかまでは
  対応が取れないため、**検証後の起動**は 1 度だけ差し戻し、「調査のみで変更なし」と
  報告に明示すれば通る(記録が無ければ起動だけで差し戻すことはない)
...
判定行が `PASS` なら通す(判定基準を曖昧にしないため)。合否は判定行の
**先頭トークンだけ**で決める。行全体への部分一致にすると、
`FAIL AC-PASSWORD-VALIDATION` のように detail へ `PASS` を含むだけの不合格が
合格扱いになる(回帰テストあり)。
...
機械で止めることはできない(防御は `agents/verifier.md` の指示力だけ)。同様に、
検証後のサブエージェント起動は「調査のみ」と述べれば通る。ハーネス全体がこの線引き
(黙って省くのは止まる、偽って通るのは止まらない)の上に立っている。
