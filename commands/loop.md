...
5. **実装に着手する前に** `/crystal:goal` と同じ手順で `.claude/goal.md` を作成する。
   仕様の AC-* を DC-* に取り込む(`max_minutes` は**書かない**。goal-gate が `LOOP.md` の
   `max_minutes_per_run` で埋める)。**必ず `status: active` で作り、自分で `done` に
   ...
   **何のメッセージも出さずに素通しする**ため、内側ループが一度も動かないまま実装だけが
   進む(Q-8 で実際に起きた)。古いゴールの `started_epoch` を引き継いだ場合は、
   初回ラウンドで即座に予算超過と判定されて `stalled` に落ちる。
